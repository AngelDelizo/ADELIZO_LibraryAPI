...
//USER CRUD
// Create/Add/Register a new user
$app->post('/user/register', function (Request $request, Response $response, array $args) {
    ...
    if (empty($usr) || empty($pass) || strlen($usr) < 3 || strlen($pass) < 3) {
        $response->getBody()->write(json_encode(array("status" => "fail", "data" => "Username and password must be at least 3 characters long and not empty")));
...
// View/Get all user
$app->get('/users/read', function (Request $request, Response $response, array $args) {
    $servername = "localhost";
    $username = "root";
    $password = "";
    $dbname = "adelizolibrary";
...
// Update user info (name and password)
$app->put('/user/update', function (Request $request, Response $response) {
    ...
    if (empty($newUsername) || empty($newPassword) || strlen($newUsername) < 3 || strlen($newPassword) < 3) {
...
$app->post('/author/add', function (Request $request, Response $response, array $args) {
    ...
    if (empty($name) || strlen($name) <= 5) {
        return $response->getBody()->write(json_encode(array("status" => "fail", "data" => "Author name must be more than 5 characters and cannot be blank.")));
    }
...
$app->post('/book/add', function (Request $request, Response $response, array $args) {
    ...
    if (empty($title) || strlen($title) <= 5) {
        return $response->getBody()->write(json_encode(array("status" => "fail", "data" => "Book title must be more than 5 characters and cannot be blank.")));
    }
...
$app->put('/book/update', function (Request $request, Response $response, array $args) {
    ...
    // Validate input
    if (empty($newTitle) || strlen($newTitle) <= 5) {
        return $response->getBody()->write(json_encode(array("status" => "fail", "data" => "Book title must be more than 5 characters and cannot be blank.")));
    }
...
//BOOK & AUTHOR RELATIONSHIP CRUD
// Create a new book-author relationship
$app->post('/book_author/add', function (Request $request, Response $response, array $args) {
    $data = json_decode($request->getBody());
    $bookId = $data->bookid;
...
// Get/View all book-author relationships
...
$app->put('/book_author/update', function (Request $request, Response $response, array $args) {
    $data = json_decode($request->getBody());
